...
def evaluate():
    ...
    if option == "categorical":
        ...
        # Convert the list of items to a dictionary (first occurrence per category)
        parsed_data = {}
        for item in items:
        ...
        # Use the previously working Gemini prompt
        prompt = (
            "You will be provided input which will contain one or more items. Convert the input into a dictionary "
            "where the category is one of the following: Grooming, Clothing, Electronics, Groceries, Medicine, Meat "
            "and then the item. The input will contain items and they need to be categorized in this format: "
            "{Category1: Item1, Category2: Item2, ...} "
            f"Input: {manual_input}"
        )
        ...
        # Use a regex that simply captures everything between the first { and the last }
        match = re.search(r"(\{[\s\S]*\})", gemini_output)
        if not match:
